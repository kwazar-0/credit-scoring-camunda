...
|---|--------|-----------------|---------------|
| **1** | **Cloud + IaC (dev)** | APIs enabled, Pulumi `pulumi up` on **dev**, GCS, BQ dataset, Artifact Registry, stack exports | [infra/pulumi/README.md](https://github.com/OlehKondratow/credit-scoring-camunda/blob/develop/infra/pulumi/README.md), [cli-console.md](cli-console.md), [scripts/gcp-enable-apis-iam.sh](https://github.com/OlehKondratow/credit-scoring-camunda/blob/develop/scripts/gcp-enable-apis-iam.sh) |
...
| **3** | **GKE + images** | Cluster (Standard), workload in namespace, images from Artifact Registry, Workload Identity for pods | [infra/ARCHITECTURE.md](https://github.com/OlehKondratow/credit-scoring-camunda/blob/develop/infra/ARCHITECTURE.md), `k8s/hbg/` |
...
| **6** | **Observability / policy** | Logs, BQ analytics without raw PII, roles matrix if needed | [infra/ROLES.md](https://github.com/OlehKondratow/credit-scoring-camunda/blob/develop/infra/ROLES.md) |
...
2. **[infra/ARCHITECTURE.md](https://github.com/OlehKondratow/credit-scoring-camunda/blob/develop/infra/ARCHITECTURE.md)** — isolation, OIDC, state.  
3. **[infra/pulumi/README.md](https://github.com/OlehKondratow/credit-scoring-camunda/blob/develop/infra/pulumi/README.md) + `__main__.py`** — what is created in the cloud.  
...
A **second** Pulumi project: [`infra/pulumi/gke-infra/`](https://github.com/OlehKondratow/credit-scoring-camunda/tree/develop/infra/pulumi/gke-infra). Same content also lives in the repo as [manual.md](https://github.com/OlehKondratow/credit-scoring-camunda/blob/develop/infra/pulumi/gke-infra/manual.md) / [manual.en.md](https://github.com/OlehKondratow/credit-scoring-camunda/blob/develop/infra/pulumi/gke-infra/manual.en.md).

The stack uses **`europe-west1`** and `credit-scoring-*` names, which **differ** from **`infra/pulumi/`** (`europe-central2`, `hbg-*`). Do **not** run both `pulumi up` in the same project without a plan: [infra/pulumi/README.md](https://github.com/OlehKondratow/credit-scoring-camunda/blob/develop/infra/pulumi/README.md).
