...
* Секрет `secrets.GCP_SA_DEV` (в YAML workflow — внутри выражения GitHub Actions для `development`) доступен только в окружении `development`.
* Секрет `secrets.GCP_SA_PROD` — только в окружении `production` и защищён `Required Reviewers`.
