...
**`Credit-Scoring-V2`** не используем как «официальное» имя продукта — это устаревший технический ярлык. Публичное имя контура: **Millennium Credit** (см. `prompt.md`).
...
- Не делать **default branch** с именем `Credit-Scoring-V2`. Legacy-ветка **`millennium-credit-v2`** удаляется после смены default — см. **[branch-notes.md](branch-notes.md)**.
