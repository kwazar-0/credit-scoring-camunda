# Handlowy Bank Galicyjski (HBG) — вымышленный пример — контекст для продолжения работы

Краткий handoff для нового чата: что за проект, как устроен **`infra/`**, Git, ограничения и что делать дальше.

**Фокус «сделать инфраструктуру Camunda + AI»:** не читайте весь файл подряд — сначала **[INFRA-IMPLEMENTATION.md](INFRA-IMPLEMENTATION.md)** (фазы) и **[toc.md](toc.md)** (оглавление). Ниже §1–8 — продукт и план; **§9+** — расширенный enterprise / hardening.
...
**Ветки, `release/*`, теги `v*`, окружения:** см. **[git-workflow.md](git-workflow.md)**.
...
| `k8s/hbg/` | Манифесты K8s. |
| `docker-compose.yml` | Локально: Zeebe + backend + worker + UI. |
| `ml-data-rag.md` | ML/Data/RAG и env backend. |
...
Команды для терминала (Docker, Pulumi, `gcloud`/`kubectl`, push образов): **[cli-console.md](cli-console.md)**.
...
- **Ветки:** **`develop`** (интеграция), **`main`** (prod), `release/*`, `feature/*`, `hotfix/*` — см. §2 и **[git-workflow.md](git-workflow.md)**.

Это итоговый, детальный промпт для настройки enterprise-инфраструктуры: облако, Kubernetes, безопасность и процессы вокруг Camunda. Используйте для Pulumi (отдельные стеки) или как инструкцию архитекторам.

### 9.1. Context & Governance
**Project:** Handlowy Bank Galicyjski (HBG) credit scoring (Camunda-based).
...
| **Человеческий доступ** | Не дублировать «левые» `roles/owner` в репо: для людей — матрица в **[gcp-saas-access-matrix-11x6.md](gcp-saas-access-matrix-11x6.md)**; фактическое **назначение** — через **группы** в GCP IAM (§9.6). |
...
3.  **Namespaces:** например `credit-dev`, `credit-ref`, `credit-prod` (в репо сейчас пример — `hbg`; согласуйте имена).
...
* **Orchestration:** GKE Standard; неймспейсы согласовать с кластером (например `credit-dev` / `credit-ref` / `credit-prod` или `hbg` в манифестах — единообразно в Pulumi и `k8s/`).
...
Ниже — **строгий англоязычный промпт** для генерации политик, IaC и CI/CD (SoT через группы, без JSON-ключей, supply chain). Согласуйте имена неймспейсов (`credit-*` vs `hbg`) с `k8s/` и §9.4.
...
* Prefer Pulumi (this repository) for infrastructure
