# Документация — оглавление

## LEVEL 0 — Вход (WHAT)

- [Вход в систему (main)](main.md)
- [Сводка системы (1 страница)](system-summary.md)
- [Архитектура (1 страница)](architecture.md)

## LEVEL 1 — Система (HOW IT WORKS)

- [Camunda process flow](process-flow-camunda.md)
- [DMN decision model](decision-model-dmn.md)
- [Философия системы](system-philosophy-governance.md)

## LEVEL 2 — Governance (WHO)

- [GCP access matrix 11x6](gcp-saas-access-matrix-11x6.md)
- [CODEOWNERS matrix](github-codeowners-matrix.md)
- [Организация 11x6](team-11x6-organization.md)
- [Git workflow](git-workflow.md)

## LEVEL 3 — Инфраструктура (WHERE IT RUNS)

- [Pulumi and GKE runtime](pulumi-and-gke-runtime.md)
- [Pulumi IaC (детали)](infra-pulumi-iac.md)
- [Pulumi GKE sandbox](infra-pulumi-gke-sandbox.md)
- [Implementation track](INFRA-IMPLEMENTATION.md)

## LEVEL 4 — Operations (HOW IT IS RUN)

- [Deployment](ops/deployment.md)
- [CI/CD](ops/cicd.md)
- [Observability](ops/observability.md)
- [Incidents and recovery](ops/incidents.md)
- [Camunda в GKE + Desktop Modeler (Ubuntu)](camunda-gke-deploy-modeler.md)

## Референсы

- [CLI / console operations](cli-console.md)
- [Приложение](appendix.md)

---

Другие языки: [English](/en/toc) · [Polski](/pl/toc)
